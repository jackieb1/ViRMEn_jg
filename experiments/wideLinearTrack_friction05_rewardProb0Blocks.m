...
%     vr.exper.variables.startYLocation = vr.startYLocation;
    ...
    % Block parameters
    vr.rewardBlockSize = eval(vr.exper.variables.rewardBlockSize);
    vr.noRewardBlockSize = eval(vr.exper.variables.noRewardBlockSize);
    vr.blockTransitionMinAccuracy = eval(vr.exper.variables.blockTransitionMinAccuracy);
    vr.inRewardBlock = true;
    vr.blockTrials = 0;
    vr.accuracyWindow = eval(vr.exper.variables.accuracyWindow);
...
function vr = runtimeCodeFun(vr)
    ...
    if vr.position(2) > vr.teleportYLocation
        ...
        vr.blockTrials = vr.blockTrials + 1;
        if vr.inRewardBlock
            if (vr.blockTrials >= vr.rewardBlockSize)
                accuracy = mean(vr.correctTrials(end-vr.accuracyWindow:end));
                if accuracy >= vr.blockTransitionMinAccuracy
                    vr.inRewardBlock = false;
                    vr.blockTrials = 0;
                    disp('Start reward omission block.')
                end
            end
        else % not in reward blck
            if (vr.blockTrials >= vr.noRewardBlockSize)
                vr.inRewardBlock = true;
                vr.blockTrials = 0;
                disp('Start reward block.')
            end
        end
    ...
    vr = collectBehaviorIter_TMaze(vr, vr.dpGain, vr.inRewardBlock);
    vr = checkForManualReward(vr); % Deliver reward if 'r' key pressed
    
    vr.distToRewardZone = hypot(vr.position(1)-vr.rewardXLocation, vr.position(2)-vr.rewardYLocation);
    if (vr.distToRewardZone < vr.rewardZoneRadius) && (vr.numRewardsThisTrial < vr.numRewardsPerTrial) && (vr.inRewardBlock)
