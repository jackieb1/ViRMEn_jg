function code = linearTrack200vu_2cues_popup_CueB100_randomRewardProb35_2
...
function vr = runtimeCodeFun(vr)
    ...
    if (~vr.cueHasAppeared) && (vr.position(2) > vr.CueRevealPosition)
        ...
        disp(['Switched to world ' num2str(vr.currentWorld), ' Num in block ', num2str(vr.numInBlock)])
        if (vr.numTrials >= vr.firstRewardedTrial)
            vr.isRewardTrial = 1;
