function code = linearTrack_velocityPulse_ForwAndBack_wideGaussStrong_noNeg_noR
...
%         vr = updateLivePlots_linearMaze(vr);
        ...
        if (rand <= vr.fractionTrials) && ~(vr.inDriftPulse)
            vr.isHeadingPulseTrial = 1;
            vr.driftYTrigger = vr.driftYPos;
            % Randomly flip pulse going forward or backward
            pulseDirection = randsample([-1 1], 1);
            if rand <= 0
                vr.driftPulseHeight = vr.driftPulseHeight_const * pulseDirection * 0.5;
            else
    ...
    if (vr.velocity(1)< 0) || (vr.velocity(2)< 0)
        vr.velocity(1) = 0;
        vr.velocity(2) = 0;
    end
