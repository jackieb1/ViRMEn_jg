function code = wideLinearTrack_friction05_ITI_towerPopup_ITIstim_rightToCenter
...
function vr = runtimeCodeFun(vr)
    ...
    if isTrialStart(vr)
        ...
        if vr.numTrials > vr.maxTrials
            vr.experimentEnded = true;
        end

        % Always start in world 1 (tower not visible)
        vr.nextWorld = 1;
        vr = initTrial(vr);
...
%             vr.blockTrial = 1;
    ...
    vr = checkforTrialEndPosition_wideLinearTrack_towerPopup(vr);
    vr = waitForNextTrial(vr);

    if vr.stimBlock && (vr.inITI || (vr.position(2) > vr.towerPopupLocation) )
        vr.optoOn = 1;
    else
        vr.optoOn = 0;
    end
