function code = T_maze_kink_stimYRange
% T_maze   Code for the ViRMEn experiment T_maze.
%   code = T_maze   Returns handles to the functions that ViRMEn
%   executes during engine initialization, runtime and termination.

% Begin header code - DO NOT EDIT
code.initialization = @initializationCodeFun;
code.runtime = @runtimeCodeFun;
code.termination = @terminationCodeFun;
% End header code - DO NOT EDIT

% --- INITIALIZATION code: executes before the ViRMEn engine starts.
function vr = initializationCodeFun(vr)
vr.debugMode = false;
vr.ops = getRigInfo();
vr = makeVirmenDir(vr);
vr = initTMaze(vr);
vr = initDAQ(vr);
vr = initLivePlots(vr);
subplot(vr.livePlot_opt.nRows, vr.livePlot_opt.nSubplots, 6)
text(0, -7, ['Time outs: 0'])

% Friction coefficient
vr.friction = 0.8;

% Make sure no cue switch on first trial
vr.currentWorld = randsample(1:2, 1);

% Photostimulation parameters
vr.optoYMin = 100;
vr.optoYMax = 220;
vr.isStimTrial = 1;
vr.stimOn = 0;
vr.maxStimOnTime_s = 30;
vr.nTimeOutTrials = 0;

% --- RUNTIME code: executes on every iteration of the ViRMEn engine.
function vr = runtimeCodeFun(vr)
if isTrialStart(vr)
    vr = updateLivePlots(vr);
    % Add line for number of timeout trials
    subplot(vr.livePlot_opt.nRows, vr.livePlot_opt.nSubplots, 6)
    text(0, -7, ['Time outs: ' num2str(sum(vr.nTimeOutTrials))])
    
    if vr.numRewards >= vr.maxNumRewards
        vr.experimentEnded = true;
    end
    
    vr = initTrial(vr);
    vr.isStimTrial = 1;
    disp('Stim trial.')
    vr.stimOn = 0;
    vr.optoWasOn = 0;
end

vr = stimYRange(vr);
vr = outputVirmenTrigger(vr);
vr = collectBehaviorIter_TMaze(vr);
vr = adjustFriction(vr);
vr = checkForManualReward(vr); % Deliver reward if 'r' key pressed
vr = checkforTrialEndPosition_Tmaze(vr);
vr = waitForNextTrial(vr);


% --- TERMINATION code: executes after the ViRMEn engine stops.
function vr = terminationCodeFun(vr)
vr = updateLivePlots(vr);
% Add line for number of timeout trials
subplot(vr.livePlot_opt.nRows, vr.livePlot_opt.nSubplots, 6)
text(0, -7, ['Time outs: ' num2str(sum(vr.nTimeOutTrials))])

vr = clearAnalogChannels(vr);
savefig(vr.performanceFig, fullfile(vr.fullPath, 'performance.fig'));
saveas(vr.performanceFig, fullfile(vr.fullPath, 'performance.pdf'));
if vr.numTrials > 0
    [vr,sessionData] = collectTrialData(vr);
end
printSessionStats(vr);

