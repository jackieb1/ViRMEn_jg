function code = wideLinearTrack_friction05_3towerPopup
...
%     vr.exper.variables.startYLocation = vr.startYLocation;
    ...
    % Tower popup
    vr.towerPopupLocation = 100;
    vr.probTowerJump = 0.2;
    vr.currentWorld = 2;
    vr.worldBlock = 2;
...
function vr = runtimeCodeFun(vr)
    ...
    if vr.position(2) > vr.teleportYLocation
        ...
        vr.worldBlock = randsample([2 3 4], 1, true, [1-vr.probTowerJump vr.probTowerJump/2 vr.probTowerJump/2]);
        disp(vr.worldBlock)
        % Set reward location
        if vr.worldBlock == 2
            vr.rewardXLocation = 0;
