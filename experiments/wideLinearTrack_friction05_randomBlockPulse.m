...
%     vr.exper.variables.startYLocation = vr.startYLocation;
    ...
    vr.inPulse = 0;
    vr.sideOffset = 0;
    vr.pulseFractionTrials = 0.75;
    vr.pulseSd = 0.375;
    vr.pulseMu = vr.pulseSd * 4;
    vr.pulseHeightAbs = 20;

    vr.blockSize = 50;
    vr.trialsInBlock = 0; % Initialize trials counter within the block
    vr.isPulseBlock = 0; % initialize pulse block
...
function vr = runtimeCodeFun(vr)
    ...
    if vr.position(2) > vr.teleportYLocation
        ...
        if vr.trialsInBlock >= vr.blockSize
            % Reset trialsInBlock counter at the beginning of each block
            vr.trialsInBlock = 0;
            % Toggle between pulse and no pulse blocks
            vr.isPulseBlock = ~vr.isPulseBlock;
            % Randomly choose pulse direction at the beginning of each pulse block
            pulseDirection = (randi([0, 1]) - 0.5) * 2; % Randomly choose -1 or 1
            vr.pulseHeight = vr.pulseHeightAbs * pulseDirection;
        end

        if vr.isPulseBlock
           vr.isPulseTrial = rand <= vr.pulseFractionTrials;
        else
            vr.isPulseTrial = false;
        end

        vr.trialsInBlock = vr.trialsInBlock + 1;
        vr.inDriftPulse = 0;
        vr.sideOffset = 0;
        vr.pulseWasTriggered = 0;
    ...
    vr = collectBehaviorIter_TMaze(vr, vr.isPulseTrial, vr.inPulse, vr.sideOffset, vr.dpGain);
