function code = linearTrack200vu_2cues_popup_rewardBlock20
...
%     vr = wrapLinearWorld(vr, 2, 1);
    ...
    firstBlockSize = 100;
    blockSize = 20;
    rewardedWorldBlockSet = [0 2 0 3];
    vr.rewardedWorld = rewardedWorldBlockSet(1);
    blockSetSize = length(rewardedWorldBlockSet);
    blockSetReps = 50; % many trials so will not go over
...
function vr = runtimeCodeFun(vr)
    ...
    if vr.position(2) > vr.mazeLength
        ...
        disp(['Rewarded World: ' num2str(vr.rewardedWorld)])
    end
    ...
    if (~vr.cueHasAppeared) && (vr.position(2) > vr.CueRevealPosition)
        ...
        disp(['Current World: ' num2str(vr.currentWorld)])
        vr.cueHasAppeared = 1;
    ...
    vr = collectBehaviorIter_TMaze(vr, vr.rewardedWorld);
