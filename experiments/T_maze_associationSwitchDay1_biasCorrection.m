function code = T_maze_associationSwitchDay1_biasCorrection
...
if isTrialStart(vr)
    ...
    if vr.numTrials == vr.trialSwitch
        disp(['Switched association after trial ' num2str(vr.numTrials)]);
        vr.worldsAvailable = [3 4];
        vr.nextWorld = chooseNextWorld(vr);
        vr.biasCorrection = true;
    end
    
    % restrict to switch worlds after bias correction is implemented
    if (vr.numTrials > vr.trialSwitch) && (vr.nextWorld < 3)
        vr.nextWorld = vr.nextWorld + 2;
    end
    vr = initTrial(vr);
end
