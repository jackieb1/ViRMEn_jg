function code = linearTrack_4cues_lickForReward_switchRewLoc
...
%     vr = wrapLinearWorld(vr, 2, 1);
    ...
    vr.isRewardTrial = 0;
    vr.rewardSwitchTrial = 50;
    vr.rewardsPerTrial = 1; % rewards per reward trigger
    
    vr.CueRevealPosition = 100;
    vr.cueHasAppeared = 0;
...
function vr = runtimeCodeFun(vr)
    ...
    if (~vr.cueHasAppeared) && (vr.position(2) > vr.CueRevealPosition)
        if (vr.numTrials >= vr.rewardSwitchTrial)
            vr.isRewardTrial = 1;
            vr.probAutomaticReward = 0.5;
